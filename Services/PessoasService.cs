...
namespace CosmosDBExemple.Services
{
    public class PessoasService : IPessoasService
    {
        ...
        public PessoasService() => _noSQLDataBase = new();
        public async Task<IEnumerable<Pessoa>> GetPessoas() => await _noSQLDataBase.GetAllItens(container);

        public async Task<IEnumerable<Pessoa>> GetPessoasPorNome(string name)
            => await _noSQLDataBase.GetByPredicate(container, m => m.Nome == name);
